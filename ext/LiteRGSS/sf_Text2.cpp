...
namespace
{
    ...
    void text2_addLine(sf::VertexArray& vertices, float lineLength, float lineTop, const sf::Color& color, float offset, float thickness, float outlineThickness = 0)
    {
    ...
    void text2_addGlyphQuad(sf::VertexArray& vertices, sf::Vector2f position, const sf::Color& color, const sf::Glyph& glyph, float italic, float outlineThickness = 0)
    {
...
void Text2::ensureGeometryUpdate() const
{
    ...
    for (std::size_t i = 0; i < m_string.getSize() && i < m_numberCharsToDraw; ++i)
    {
        ...
        {
            text2_addLine(m_vertices, x, y, m_fillColor, underlineOffset, underlineThickness);

            if (m_outlineThickness != 0)
                text2_addLine(m_outlineVertices, x, y, m_outlineColor, underlineOffset, underlineThickness, m_outlineThickness);
        }

        // If we're using the strike through style and there's a new line, draw a line across all characters
        if (strikeThrough && (curChar == L'\n'))
        {
            text2_addLine(m_vertices, x, y, m_fillColor, strikeThroughOffset, underlineThickness);

            if (m_outlineThickness != 0)
                text2_addLine(m_outlineVertices, x, y, m_outlineColor, strikeThroughOffset, underlineThickness, m_outlineThickness);
        }
        ...
        if(m_DrawShadow)
        {
            // Extract the current glyph's description
            const Glyph& glyph = m_font->getGlyph(curChar, m_characterSize, bold);
            text2_addGlyphQuad(m_outlineVertices, Vector2f(x + 1, y), m_outlineColor, glyph, italic);
            text2_addGlyphQuad(m_outlineVertices, Vector2f(x + 1, y + 1), m_outlineColor, glyph, italic);
            text2_addGlyphQuad(m_outlineVertices, Vector2f(x, y + 1), m_outlineColor, glyph, italic);
        }
        // Apply the outline
        else if (m_outlineThickness != 0)
        {
            ...
            text2_addGlyphQuad(m_outlineVertices, Vector2f(x, y), m_outlineColor, glyph, italic, m_outlineThickness);
